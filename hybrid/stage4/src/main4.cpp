...
template<typename T> void adnn_init_bias(const caffe::Blob<T>& blob);
template<typename T> void adnn_init_weight(const caffe::Blob<T>& blob);
...
extern int adnn_run_forward(alib_obj lib_handle);    // define in AdnnRunLayer.cpp
#endif
  
#ifdef TEST_INFER
// define in AdnnConvLayerInfer.cpp
template <typename T> int adnn_init_layer_infer(alib_obj lib_handle, const caffe::Blob<T> *src, caffe::Blob<T> *des);
extern int adnn_run_forward(alib_obj lib_handle);
