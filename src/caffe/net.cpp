...
template <typename Dtype>
Net<Dtype>::Net(const string& param_file, Phase phase) // TT: a solver or prototxt file such as train_val.prototxt 
{
  NetParameter param;
...
void Net<Dtype>::Init(const NetParameter& in_param)
{
  ...
  if (param.input_dim_size() > 0)
    ...
    {
      CHECK_EQ(param.input_size(), param.input_shape_size())   << "Exactly one input_shape must be specified per input.";
    }
	  ...
	  for (int bottom_id = 0; bottom_id < bottom_need_backward_[layer_id].size(); ++bottom_id)
	    {
	      ...
	      blob_need_backward_[bottom_id_vecs_[layer_id][bottom_id]] = blob_need_backward_[bottom_id_vecs_[layer_id][bottom_id]]
...
// Helper for Net::Init: add a new input or top blob to the net.  (Inputs have layer_id == -1, tops have layer_id >= 0.)
template <typename Dtype>
			   ...
			   map<string, int>* blob_name_to_idx)
{
  ...
  if (blob_name_to_idx && blob_name_to_idx->find(blob_name) != blob_name_to_idx->end())
    {
			     ...
			     map<string, int>* blob_name_to_idx)
{
  ...
  if (available_blobs->find(blob_name) == available_blobs->end())
    {
      LOG(FATAL) << "Unknown blob input " << blob_name << " (at index " << bottom_id << ") to layer " << layer_id;
    }
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,  const int param_id)
{
  ...
  if (!param_size || !param_name.size() || (param_name.size()  && param_names_index_.find(param_name) == param_names_index_.end()))
  ...
  else
    {
      ...
      LOG(INFO) << "Sharing parameters '" << param_name << "' owned by layer '" << layer_names_[owner_layer_id]
      ...
      else
	{
      ...
      layers_[layer_id]->blobs()[param_id]->ShareData(*layers_[owner_layer_id]->blobs()[owner_param_id]);
  }
}
  ...
  for (int i = 0; i < layers_.size(); ++i)
      ...
      for (int j = 0; j < layer_blobs.size(); ++j)
	{
	  const ParamSpec* param_spec =
	    (layers_[i]->layer_param().param_size() > j) ?  &layers_[i]->layer_param().param(j) : &default_param_spec;
