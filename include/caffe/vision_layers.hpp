...
namespace caffe {
  ...
  //  class ConvolutionLayer; // TT: PATCH 160318
...
class BaseConvolutionLayer: public Layer<Dtype>
{
  ...
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);  // Stage4.9 - creating a Setup in ConvolutionLayer
  ...
  void ocl_setup();
    ...
    void forward_gpu_gemm_opt(const Dtype* col_input, const Dtype* weights, Dtype* output, bool skip_im2col = false);
    ...
    void backward_gpu_gemm_opt(const Dtype* input, const Dtype* weights, Dtype* col_output);
    void weight_gpu_gemm(const Dtype* col_input, const Dtype* output, Dtype* weights);
    void weight_gpu_gemm_opt(const Dtype* col_input, const Dtype* output, Dtype* weights);
  ...
  {
    im2col_gpu(data, bottom_offset_, conv_in_channels_, conv_in_height_, conv_in_width_, kernel_h_, kernel_w_, pad_h_, pad_w_,
	       stride_h_, stride_w_, col_buff, 0);
  }
  
  inline void conv_col2im_gpu(const Dtype* col_buff, Dtype* data)
  {
    col2im_gpu(col_buff, 0, conv_in_channels_, conv_in_height_,  conv_in_width_, kernel_h_, kernel_w_, pad_h_, pad_w_, stride_h_,
...
protected:
  ...
  {
    im2col_gpu_opt(data, bottom_offset_, conv_in_channels_, conv_in_height_, conv_in_width_, kernel_h_, kernel_w_, pad_h_, pad_w_,
		   stride_h_, stride_w_, (Dtype*) transMem, 0, opt_num2);
  }
  
  inline void conv_col2im_gpu_opt(Dtype* data)
  {
    col2im_gpu_opt((Dtype*) transMem, 0, conv_in_channels_, conv_in_height_, conv_in_width_, kernel_h_, kernel_w_, pad_h_, pad_w_,
		   stride_h_, stride_w_, data, bottom_offset_,  opt_num2);
  }
...
public:
    ...
    explicit ConvolutionLayer(const LayerParameter& param) : BaseConvolutionLayer<Dtype>(param)  {}
  ...
  virtual bool reverse_dimensions() { return false; }   // TT: removed inline
  virtual void compute_output_shape();


  // 160316 : Stage4.9
  // Creating this SetupLayer to call the AdnnSetupLayer (bridge)
  // It will call the super::Reshape and then it will call AdnnSetupLayer
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);
  
#ifndef CPU_ONLY
  // Stage4 : see repo or archive for these - nopt required for ADNN integration
  // virtual void Forward_gpu_org(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);
  // virtual void Backward_gpu_org(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  // virtual void Forward_gpu_batched(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);
  // virtual void Backward_gpu_batched(const vector<Blob<Dtype>*>& top,const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
#endif
};
...
{
public:
  explicit DeconvolutionLayer(const LayerParameter& param) : BaseConvolutionLayer<Dtype>(param)  { }
...
public:
  ...
  explicit Im2colLayer(const LayerParameter& param) : Layer<Dtype>(param) { }
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,  const vector<Blob<Dtype>*>& top);
